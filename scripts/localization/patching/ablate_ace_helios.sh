...
for feature in "${FEATURES[@]}"; do
    for block in "${LAYERS_TO_PATCH[@]}"; do
        ...
        log_file="slurm_out/${MODEL_NAME}/${feature}/ablate_${block}.log"
...
accelerate launch --num_processes 4 --main_process_port $main_process_port src/patch_layers.py experiment=ablate_${MODEL_NAME}/${MODEL_NAME}_$feature patch_layers=${MODEL_NAME}/self_attn/$block patch_config.path_with_results=$output_dir
EOT
    done
done